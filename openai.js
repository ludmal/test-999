...
const CONFIG = {
  ...
  OPENAI_API_KEY: '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',
  
  // Your Workflow ID from Agent Builder (starts with wf_)
  WORKFLOW_ID: 'wf_68e627beb3008190ae5204d8ab71c16c0c33a45c233669ed',
  
  // Domain Key (get from: https://platform.openai.com/settings/organization/security/domain-allowlist)
  // REQUIRED for production domains. Leave empty only if using Draft workflow for testing
  DOMAIN_KEY: '', 
  
  // Set to true if you're testing with a Draft workflow (bypasses domain verification)
  USE_DRAFT_MODE: false,
  
  // ChatKit container settings
...
(function() {
  ...
  async function createSession() {
    ...
    sessionPromise = (async () => {
      try {
        ...
        console.log('API Key format:', CONFIG.OPENAI_API_KEY.substring(0, 8) + '...');
  ...
  function createPanelUI() {
    ...
    const existingContainers = document.querySelectorAll('#' + CONFIG.CONTAINER_ID);
    ...
    document.body.style.marginRight = CONFIG.PANEL_WIDTH; // START WITH MARGIN
    toggleBtn.style.right = `calc(20px + ${CONFIG.PANEL_WIDTH})`; // START WITH BUTTON MOVED
    ...
    toggleBtn.addEventListener('click', () => {
      ...
      if (isOpen) {
        panel.style.right = '0';
        document.body.style.marginRight = CONFIG.PANEL_WIDTH;
        toggleBtn.style.right = `calc(20px + ${CONFIG.PANEL_WIDTH})`;
      } else {
        panel.style.right = `-${CONFIG.PANEL_WIDTH}`;
  ...
  async function initializeChatKit() {
    try {
      ...
      // Add domain key if configured (required for production)
      if (CONFIG.DOMAIN_KEY) {
        options.api.domainKey = CONFIG.DOMAIN_KEY;
        console.log('🔑 Using domain key for verification');
      }
